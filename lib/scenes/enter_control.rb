...
class EnterControl < DialogScene
  ...
  def setup(previous_scene, message, current)
    ...
    gui_controls << Button.new(t.button.ok.string, at: [width * 0.37, 40], size: 8, auto_center: [0.5, 0], shortcut: nil) do
      pop_scene @key
    end

    gui_controls << Button.new(t.button.cancel.string, at: [width * 0.63, 40], size: 8, auto_center: [0.5, 0], shortcut: nil) do
      pop_scene nil
    end
